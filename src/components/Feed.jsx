...
export default function Feed() {
  ...
  const sendIntrestRequest = async (status, profile) => {
    console.log("profile==>", { profile });
    const url =
      BASE_URL + "/connectionRequest/send/" + status + "/" + profile?._id;

    const res = await axios.post(url, {}, { withCredentials: true });
    console.log("resss==>", { url, res });
    if (res.status) {
      let filteredFeed = profiles.filter((p) => p?._id !== profile?._id);
        ...
        {profile && (
          <div className="w-[380px] bg-white rounded-3xl shadow-2xl overflow-hidden relative">
            ...
            <div className="absolute bottom-5 left-40 right-0 flex justify-center gap-8">
              <button
                onClick={() => sendIntrestRequest("ignore", profile)}
              ...
              <button
                onClick={() => sendIntrestRequest("interested", profile)}
